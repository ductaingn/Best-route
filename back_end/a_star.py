...
def a_star(start_point,end_point):
    ...
    start_node_data = node_df.loc[node_df['osmid']==start_node_id,['osmid','y','x']].to_numpy()[0]
    end_node_data = node_df.loc[node_df['osmid']==end_node_id,['osmid','y','x']].to_numpy()[0]
    ...
    while(len(boundary_nodes)>0):
        current_node_data = np.array(min(boundary_nodes,key=boundary_nodes.get))
        ...
        adjacent_edge_data = edge_df.loc[edge_df['u']==current_node_data[0],['osmid','u','v','length','geometry']].to_numpy()

        for num_of_adj in range(adjacent_edge_data.shape[0]):
            node_data = node_df.loc[node_df['osmid']==adjacent_edge_data[num_of_adj][2],['osmid','y','x']].to_numpy()[0]
            ...
            g=distance.get(current_node_data[0]) + adjacent_edge_data[num_of_adj][3]
